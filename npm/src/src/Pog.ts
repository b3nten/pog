type TParseInput<T extends string> = {
...
export class Pog<M extends string, A extends string> {
    ...
    do {
      ...
      content = content.replace(
        ...
        (from, pre, sep, body: string) => {
          if (!seperators.includes(sep)) return from;
          return body
            .split(/\s/g)
            .filter(Boolean)
            .map((i) =>
              i === "~" ? pre : i.replace(/^(!?)(.*)/, `$1${pre}${sep}$2`)
            )
            .join(" ");
        }
      );
  ...
  protected addKeyToString(key: string, value: string, modifier = true) {
    return (
      key +
      (key.length ? (modifier ? ":" : "-") : '') +
      this.formatString(value).replace(
        /\s+/g,
        " " + key + (key.length ? (modifier ? ":" : "-") : '')
      )
    );
  }

  protected formatArray(arr: Array<string | Record<string, unknown>>) {
    const acc: string[] = [];
    for (const element of arr) {
      if (typeof element === "string") {
        ...
        acc.push(
          this.parse(element as TParseInput<M>)
        );
      }
    }
    return acc.join(" ");
  ...
  protected addKeyToArray(
    ...
    for (const element of arr) {
      if (typeof element === "string") {
        ...
        acc.push(
          key + (key.length ? (modifier ? ":" : "-") : '') + this.parse(element as TParseInput<M>)
        );
      }
    }
    return acc.join(" ");
  ...
  public parse(
    content: TParseInput<M> | string | Array<string | TParseInput<M>>,
    modifier = true
  ){
    // if the content is a string, return the formatted string
    if (typeof content === "string") return this.formatString(content);
    // if the content is an array, return the formatted array
    if (Array.isArray(content)) return this.formatArray(content)
    ...
    for (const [key, value] of Object.entries(content)) {
      ...
      if (typeof value === "object") {
        acc.push(
          key + (modifier ? ":" : "-") + this.parse(value as typeof content)
        );
        continue;
      }
    }
